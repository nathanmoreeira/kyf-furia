...
<header style="height: 7%; display: flex; justify-content: space-between; align-items: center; padding: 0 20px; background-color: #1a1a1a;">
  ...
  <form method="GET" action="{{ route('dashboard') }}" style="display: flex; flex-wrap: wrap; gap: 15px;">
    <div class="input-group" style="flex: 1 1 30%; min-width: 200px;">
      ...
      <input type="text" id="name" name="name" value="{{ request('name') }}" placeholder="Digite o nome" style="width: 100%; padding: 10px; border: none; border-radius: 8px; background: #333; color: #fff; font-size: 14px;" />
    </div>

    <div class="input-group" style="flex: 1 1 30%; min-width: 200px;">
      <label for="favorite_sport" style="display: block; color: #ccc; margin-bottom: 5px;">E-Sport favorito</label>
      <input type="text" id="favorite_sport" name="favorite_sport" value="{{ request('favorite_sport') }}" placeholder="E-Sport favorito" style="width: 100%; padding: 10px; border: none; border-radius: 8px; background: #333; color: #fff; font-size: 14px;" />
    </div>

    <div class="input-group" style="flex: 1 1 30%; min-width: 200px;">
      <label for="favorite_player" style="display: block; color: #ccc; margin-bottom: 5px;">Jogador favorito</label>
      <input type="text" id="favorite_player" name="favorite_player" value="{{ request('favorite_player') }}" placeholder="Jogador favorito" style="width: 100%; padding: 10px; border: none; border-radius: 8px; background: #333; color: #fff; font-size: 14px;" />
    </div>

    <div style="flex: 1 1 100%; text-align: center; margin-top: 20px;">
      <button type="submit" class="filter-button" style="background: #8a2be2; color: white; padding: 10px 20px; border-radius: 8px; border: none; cursor: pointer; font-size: 16px; width: 13%;">
        Filtrar
      </button>
    </div>
  </form>
            ...
            <tr>
                ...
                <td style="display: flex; gap: 10px;">
                        ...
                        <form action="" method="POST" style="display: inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn-danger" style="background: none; border: none; cursor: pointer; color: crimson; font-size: 18px;">
